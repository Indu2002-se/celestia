...
const Home = () => {
  ...
  useEffect(() => {
    const fetchFeaturedEvents = async () => {
      try {
        ...
        // If we have real data, use it; otherwise fall back to placeholder
  ...
  const getEventImage = (event) => {
    ...
    return event.image_url || 'https://images.unsplash.com/photo-1489599831529-2a9e9c3b5b6c?w=800&h=600&fit=crop';
  };

  const getEventLocation = (event) => {
    // Use venue if available, otherwise use location
    return event.venue || event.location || 'Location TBD';
  };

  const getEventPrice = (event) => {
    // Check if it's a real event from database (has price field)
    if (event.price !== undefined) {
      return `LKR ${parseFloat(event.price).toFixed(2)}`;
    }
    // Fallback for placeholder events
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-dark">{getEventPrice(event)}</span>
